...
логика:
- ищем все SUG c фильтром по ExpiredOnly","SupersededOnly","ExpiredSuperseded
- из SUG можно сразу удалить просроченные
- но нужнен список просроченных (если заказана такая операация) и скаченных файлов чтобы удалить из SUP
- By_SUG_CI_ID
- By_SUG_Root
#>
